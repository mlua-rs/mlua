...
#[cfg(feature = "time")]
impl IntoLua for time::OffsetDateTime {
    fn into_lua(self, lua: &Lua) -> Result<Value> {
        ...
        match value {
            Value::String(lua_string) => {
                ...
                time::OffsetDateTime::parse(&datetime_str, &time::format_description::well_known::Rfc3339).map_err(|e| Error::FromLuaConversionError {
                    from: "string",
                    to: "time::OffsetDateTime".to_string(),
...
#[cfg(feature = "json")]
impl IntoLua for serde_json::Value {
    #[inline]
    fn into_lua(self, lua: &Lua) -> Result<Value> {
        match self {
    ...
    fn into_lua(self, lua: &Lua) -> Result<Value> {
        match self {
            ...
            serde_json::Value::Object(obj) => {
                let lua_table = lua.create_table()?;
                for (key, value) in obj {
                    lua_table.set(key.as_str(), value.into_lua(lua)?)?;
                }
                Ok(Value::Table(lua_table))
            },
        }
...
#[cfg(feature = "uuid")]
impl FromLua for uuid::Uuid {
    #[inline]
    fn from_lua(value: Value, lua: &Lua) -> Result<uuid::Uuid> {
        let ty = value.type_name();
